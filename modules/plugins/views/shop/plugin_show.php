...
<?php if( $listItem ) : ?>
						...
						<?php foreach( $listItem as $row ) : ?>
								<?php if( $row->niveau <= $user->niveau && !isset( $itemProtect[$row->id] ) ) : ?>
										<tr>
												...
												<?php endif; ?>
												<td width="40"><img src="<?php echo url::base(); ?>images/items/<?php echo $row->image; ?>" width="30" height="30" /></td>
												<td><label for="item_<?php echo $row->id; ?>"><span class="titreSpanForm">
																		<?php echo $row->name; ?>
																</span></label></td>
												<td align="left" width="220" class="<?php echo $row->hp || $row->attaque ? 'vert' : 'rouge'; ?>">
														<?php
														if( $row->hp )
		...
		<div class="formButton">
				<strong class="rouge"><?php echo Kohana::lang( 'shop.total' ); ?></strong> : <strong><span id="prix_total">0</span></strong> <?php echo Kohana::config( 'game.money' ); ?>
				...
				<input type="button" onclick="overlay.load( 'actions/shop' )" class="button button_rouge" value="<?php echo Kohana::lang( 'form.annul' ); ?>"/>
		</div>
		<?php
 endif	?>
