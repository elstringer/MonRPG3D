THREE.Person = function ( picture, data, sound, hand_left, hand_right, name ) {
		...
		this.changeLeft = function ( id ) {
				...
				if(!id  || data.items['item_'+id] == undefined || data.items['item_'+id].image == undefined ) 
						return;
				
				this.handLeft.add(this.loadItem(-30, data.items['item_'+id].image ));
		...
		this.changeRight = function ( id ) {	
				...
				this.handRight.add(this.loadItem(-45, data.items['item_'+id].image ));
		...
		this.loadItem = function ( rotation, image ) {		
				...
				var item = new THREE.Item( data, sound, image );
				item.scale.x = 0.5;
				item.scale.y = 0.5;
				item.scale.z = 0.5;		
				item.position.y = -11;		
				item.position.z = -0.5;		
				item.position.x = 5;		
				item.rotation.z = rotation * Math.PI / 180;	
				return item;
		};
